...
export class ActionQueueEngine {
	static async processToQueue(decisionId: string) {
		...
		log.step(
			'started',
			`Processing Decision ${decisionId} to Action Queue`,
			'Initialization of the Parameter Resolution engine. We are taking the strategic decision and preparing the individual work orders.'
		);
		...
		if (decision.actionQueue.length > 0) {
			log.step(
				'warning',
				`Action Queue records already exist for decision ${decisionId}. Skipping re-processing.`,
				'Guard triggered to prevent duplicate work orders for the same decision.'
			);
		...
		log.step(
			'actions_received',
			`Received ${selectedActionIds.length} unique action(s) from Orchestrator`,
			`The dispatcher has taken the deduplicated list of tasks generated by the Brain.`
		);
		...
		for (const actionId of allActionIds) {
			...
			const dueAt =
				lane === 'approval_required' ? new Date(Date.now() + 12 * 60 * 60 * 1000) : null;
			...
			log.step(
				'item_queued',
				`🎯 Queued: ${actionId} | Status: ${status.toUpperCase()} | Lane: [${lane.toUpperCase()}]`,
				`Action ${actionId} is successfully stored with trace ID ${qtraceId}.`
			);
		}
		...
		for (const blocked of blockedAuditIds) {
			...
			log.step(
				'blocked_audit',
				`Logged Blocked Action: ${blocked.id}`,
				`Preserved audit record for safety violation: ${blocked.reason}`
			);
		}

		log.step(
			'completed',
			`Successfully processed ${queueItems.length} total queue/audit records`,
			'All strategic tasks have been turned into detailed work orders or blocked audit logs.'
		);
		return { queueItems, log };
	}

	private static async determineLane(
		actionMeta: any,
		companyId: string,
		log: ActionQueueLog
	): Promise<string> {
		if (actionMeta.isPublicFacing) {
			log.step(
				'lane_assignment',
				`Action ${actionMeta.actionId} is public-facing. Forced to APPROVAL_REQUIRED per Safety Rule ORC-004.`,
				'Even if high automation was requested, the system has locked this public task into the Approval Lane for safety.'
			);
			return 'approval_required';
		}

		const clientProfile = await prisma.pipelineOrchestratorProfile.findUnique({
			where: { companyId }
		});
		if (clientProfile?.automationLevel === 'aggressive') {
			log.step(
				'lane_assignment',
				`Action ${actionMeta.actionId} set to AUTOMATIC via Aggressive automation profile.`,
				"This internal task will execute instantly based on the client's aggressive automation settings."
			);
			return 'automatic';
		}
		...
		if (clientProfile?.automationLevel === 'conservative') {
			log.step(
				'lane_assignment',
				`Action ${actionMeta.actionId} set to APPROVAL_REQUIRED via Conservative automation profile.`,
				"The client's conservative profile requires a human check even for internal tasks."
			);
			return 'approval_required';
		}
		...
		log.step(
			'lane_assignment',
			`Action ${actionMeta.actionId} using default lane: ${lane.toUpperCase()}`,
			`Applying the standard default lane for ${actionMeta.actionId}.`
		);
		return lane;
	}
		...
		for (const key of requiredParams) {
			switch (key) {
				...
				case 'rating':
					...
					break;
				case 'platform':
					params[key] = event.provider === 'google_business_profile' ? 'Google' : event.provider;
					break;
				case 'provider':
					params[key] = event.provider;
					break;
				case 'event_type':
					params[key] = event.eventType;
					break;
				case 'brand_tone':
					params[key] = businessConfig?.brandTone || 'professional';
					break;
				case 'office_hours':
					params[key] = businessConfig?.officeTimezone || 'UTC';
					break;
				case 'review_text':
					params[key] = event.reviewText || event.unstructuredText;
					break;
				case 'body_text':
					// Full original content — email body for the email draft handler,
					// call transcript for ACT-A2P-005. reviewText carries the raw
					// payload text for every event type; unstructuredText can hold
					// serialized metadata instead (unified-pipeline.ts), so reviewText
					// is preferred.
					params[key] = event.reviewText || event.unstructuredText || '';
					break;
				case 'text_content':
					params[key] = event.unstructuredText;
					break;
				case 'ai_summary':
					params[key] = enrichment.aiSummary;
					break;
				case 'sentiment':
					params[key] = enrichment.aiSentiment;
					break;
				case 'intent':
					params[key] = enrichment.aiRequestedAction || 'Unknown';
					break;
				case 'phone_number':
					params[key] = event.customerProfile?.phoneNumber || 'Unknown';
					break;
				case 'urgency_level': {
				...
				case 'complaint_topics':
					params[key] = enrichment.aiComplaintTopics
						? typeof enrichment.aiComplaintTopics === 'string'
							? JSON.parse(enrichment.aiComplaintTopics)
			...
			log.step(
				...
				`Extracted the specific ${key} data point from the event context.`
			);
		}
		return params;
	}
