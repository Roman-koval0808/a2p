...
export const load: PageServerLoad = async ({ locals }) => {
	...
	try {
		...
		console.error('Failed to load profiles from ProfileDB for sms drafter:', err);
	}
	...
	try {
		...
		console.error('Error fetching Telnyx numbers in sms drafter page load:', e);
	}
	...
	// 4. Fetch recent SMS history
	const messages = await locals.prisma.communicationLog.findMany({
		where: {
			companyId,
			type: 'sms'
		},
	...
	return {
		contacts,
		phoneNumbers,
		messages: JSON.parse(JSON.stringify(messages)) // Serialize Dates/Json safely
	};
};
...
export const actions: Actions = {
	deleteLog: async ({ request, locals }) => {
		...
		try {
			...
			return { success: false, error: err.message || 'Failed to delete message log' };
		}
	}
};