...
import { getNextBucket, calculateDecayedScore } from '../services/scoring.service';
 ...
 */
export async function ingestTelemetryEvent(req: Request, res: Response) {
  ...
  try {
    ...
    let fingerprintId: string = req.body.fingerprintId;

    const eventTime = occurredAt ? new Date(occurredAt) : new Date();

    // Determine Provider Name
    let providerName = req.body.provider || payload.provider;
    if (!providerName) {
    ...
    if (typeof tenantSlug !== 'string' || tenantSlug.trim() === '') {
      ...
      return res.status(400).json({ error: 'tenantSlug is required and must be a string.', trace_id: traceId });
    }
    if (typeof eventType !== 'string' || eventType.trim() === '') {
      stageLog('Q1', 'REJECT — eventType missing or invalid');
      return res.status(400).json({ error: 'eventType is required and must be a string.', trace_id: traceId });
    }
    ...
    if (!fingerprintId || typeof fingerprintId !== 'string' || fingerprintId.trim() === '') {
      ...
      if (isWebhookProvider) {
        ...
        const phoneRaw = phone || payload.phone || req.body.phone;
        const externalId = req.body.externalEventId || payload.review_id || payload.call_control_id;
        ...
        return res.status(400).json({ error: 'fingerprintId is required and must be a string.', trace_id: traceId });
      }
    } else {
    ...
    if (providerConfig) {
      ...
      for (const key of required) {
        if (req.body[key] === undefined && payload[key] === undefined && req.body.payload?.[key] === undefined) {
          return res.status(400).json({ error: `Required field ${key} is missing for provider ${providerName}` });
        }
      }
      const rating = req.body.rating !== undefined ? req.body.rating : (payload.rating !== undefined ? payload.rating : req.body.payload?.rating);
      if (rating !== undefined) {
    ...
    if (externalEventId) {
      ...
      if (existingEvent) {
        ...
        return res.status(409).json({ error: 'Duplicate event detected (permanent ID deduplication).', trace_id: traceId });
      }
      stageLog('Q1', 'Deduplication PASS — no prior record found');
    }

    // Check 3: Authentication validation
    const signature = req.headers['x-clearsky-signature'] || req.headers['x-telnyx-signature'];
    const authHeader = req.headers['authorization'];
    const isTestMode = req.body.isTest || 
    ...
    if (!isTestMode && providerConfig) {
      if (providerConfig.authType === 'webhook_signature' && !signature) {
        ...
        return res.status(401).json({ error: 'Authentication signature validation failed.', trace_id: traceId });
      }
      if (providerConfig.authType === 'client_api_key' && !authHeader) {
        stageLog('Q1', 'REJECT — API key missing');
        return res.status(401).json({ error: 'Client API key validation failed.', trace_id: traceId });
      }
    }
    ...
    const csToken = req.body.cs_token || payload.cs_token || req.body.payload?.cs_token;
    
    let resolvedGroup = req.body.group;
    let resolvedTier = req.body.tier;
...
export async function getTenantEvents(req: Request, res: Response) {
  try {
    const { tenantSlug } = req.params;
    const { page = '1', limit = '20', eventType, sessionId } = req.query;
 ...
 */
export async function clearTenantTelemetry(req: Request, res: Response) {
  try {
    const { tenantSlug } = req.params;
    let tenant = await prisma.tenant.findUnique({
      where: { slug: tenantSlug },
...
export async function sendSmsController(req: Request, res: Response) {
  try {
    ...
    if (!targetPhone) {
      return res.status(400).json({ success: false, error: 'Recipient phone number is required (no to or DEV_TEST_PHONE configured).' });
    }
    if (!body) {
      return res.status(400).json({ success: false, error: 'Body parameter is required.' });
    }

    if (!telnyxApiKey || !telnyxFrom) {
      return res.status(500).json({ success: false, error: 'Telnyx SMS is not configured in profiledb environment.' });
    }
    ...
    const reqApi = request(options, (response: any) => {
      let data = '';
      ...
      response.on('end', () => {
        if (response.statusCode && response.statusCode >= 200 && response.statusCode < 300) {
          try {
            ...
            return res.status(200).json({ success: true, sid: parsed?.data?.id });
          } catch {
            return res.status(200).json({ success: true });
          }
        }
        console.error('[Telnyx SMS API Error]:', data);
        return res.status(response.statusCode || 500).json({ success: false, error: data || 'Telnyx API error' });
      });
    });
...
async function sendAutomatedSms(to: string, body: string): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    const req = request(options, (response: any) => {
      let data = '';
      response.on('data', (chunk: any) => { data += chunk; });
      response.on('end', () => {
        if (response.statusCode && response.statusCode >= 200 && response.statusCode < 300) {
