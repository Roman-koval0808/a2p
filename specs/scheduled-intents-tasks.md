...
- [x] **Task 1.1: Add `ScheduledIntent` Model to Prisma Schema**
...
- [x] **Task 2.1: Create Extraction Types & Prompt Interface**
...
- [x] **Task 2.2: Implement LLM Extraction Logic**
...
- [x] **Task 2.3: Unit Tests for Intent Extraction**
...
- [x] **Task 3.1: Immediate Auto-Reply Sender (No Dates)**
  ...
  - **Status (5 Aug 2026): built, then REMOVED from intake by product decision** — the Orchestrator
    drafts a reply for every inbound, so the ack produced two messages per email. The module was
    deleted; the "our reply never counts as contact" rule is still enforced for the CRM note.

- [x] **Task 3.2: Dual-Record Persistence Writer**
...
- [x] **Task 4.1: Open Commitment Evaluator**
...
- [x] **Task 4.2: Modify Engagement Score Decay Logic**
...
- [x] **Task 4.3: Marketing & Nurture Suppression**
...
- [x] **Task 5.1: Create Scheduled Intents Sweep Engine**
...
- [x] **Task 5.2: Implement Type-Specific Verification Checks**
...
- [x] **Task 5.3: Hook Sweep into Daily Cron**
...
- [x] **Task 6.1: Orchestrator Handoff Bridge**
  ...
  - **Status:** implemented as `scheduled-intents-handoff.ts` — verified intents are queued as
    `pending_approval` CommunicationLog rows in the SAME approval lane the orchestrator uses,
    so existing safety gating applies without adding a new door.

- [x] **Task 6.2: Personalised Agent Draft Generator**
  - Format agent follow-up message using saved exact customer quote (e.g., *"You mentioned you'd give us a call about air conditioning in a couple of weeks — thought I'd save you the job..."*).
  - Restate only what the customer actually said — no assumptions about why or "when you're back".
  - Prohibit generic batch messaging.

- [x] **Task 6.3: Channel Fallback Resolver**
...
- [x] **Task 7.1: Schedule API Endpoint**
...
- [x] **Task 7.2: Customer Profile Schedule Component**
...
- [x] **Task 8.1: Simulation & Scenario Verification Script**
  ...
  - End-to-end test walking through Ray Charbonneau scenario:
    ...
    2. ~~Verify immediate date-free reply sent~~ — superseded 5 Aug: no instant ack; the Orchestrator drafts the reply.
